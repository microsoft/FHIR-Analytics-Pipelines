...
2. Fill the form based on the table below and click on **Review and Create** to start the deployment.
    ...
    | Scheduler Crontab Expression (new) | Use a crontab expression to set the scheduler. A crontab expression is a six-part crontab format (sec min hour day month day-of-week). Refer to [Crontab expressions for pipeline scheduler](https://github.com/atifaziz/NCrontab) to learn how to write crontab expression to schedule a job. |
