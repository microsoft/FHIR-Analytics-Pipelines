...
2. Fill the form based on the table below and click on **Review and Create** to start the deployment.
    ...
    | Customized Schema | Whether to enable the customized schema for the pipeline, the ```Customized Schema Image Reference``` need to be provided if this value been set to ```true```. |
