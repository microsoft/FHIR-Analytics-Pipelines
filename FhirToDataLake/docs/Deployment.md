...
2. Fill the form based on the table below and click on **Review and Create** to start the deployment.
    ...
    | Filter Scope | **For filter feature only** The export scope, `System` or `Group`.|
    | Group Id | The group id you want to export data for `Group` scope.|
    | Required Types | Specify which types of resources will be included. For example, _type=Patient would return only patient resources. Will export all resource types if not specified. |
    | Type Filters | Use along with the requiredTypes configuration. The value is a comma-separated list of FHIR queries that further restrict the results. Will export all data of requiredTypes if not specified. |
...
4. Refer [here](./How%20to%20use%20filtering.md) for more information about filtering.
