...
{
    public class FhirDataLakeToDataLakeSchedulerService : ISchedulerService
    {
        ...
        private readonly ILogger<FhirDataLakeToDataLakeSchedulerService> _logger;
        ...
        public FhirDataLakeToDataLakeSchedulerService(
            ...
            ILogger<FhirDataLakeToDataLakeSchedulerService> logger)
        {
        ...
        public async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            while (!scheduledToEnd && !cancellationToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    if (!_queueClient.IsInitialized() || !_metadataStore.IsInitialized() || !_completedBlobStore.IsInitialized())
                    {
        ...
        private async Task EnqueueOrchestratorJobAsync(CurrentTriggerEntity currentTriggerEntity, CancellationToken cancellationToken)
        {
            ...
            var orchestratorDefinition = new FhirDataLakeToDataLakeOrchestratorJobInputData
            {
        ...
        private async Task<bool> CreateNextTriggerAsync(CurrentTriggerEntity currentTriggerEntity, CancellationToken cancellationToken)
        {
            ...
            if (nextTriggerEndTime != null)
            {
                ...
                currentTriggerEntity.JobVersion = FhirDataLakeToDataLakeJobVersionManager.CurrentJobVersion;
        ...
        private async Task<CurrentTriggerEntity> CreateInitialTriggerEntity(CancellationToken cancellationToken)
        {
            var initialTriggerEntity = new CurrentTriggerEntity
            {
                ...
                TriggerSequenceId = 0,
                JobVersion = FhirDataLakeToDataLakeJobVersionManager.CurrentJobVersion,
            };
        ...
        /// For incremental job, lastTriggerEndTime is the end time of the last trigger, we also get all the next occurrence times during lastTriggerEndTime and utc now.
        /// If next occurrence time comes after utc now, will return null and skip this iteration.
        /// </summary>
        ...
        private DateTimeOffset? GetNextTriggerEndTime(DateTimeOffset? lastTriggerEndTime)
        {
            ...
            DateTimeOffset nextTriggerEndTime = DateTimeOffset.UtcNow;
