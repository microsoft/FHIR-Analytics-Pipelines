CREATE EXTERNAL TABLE [fhir].[Invoice] (
    ...
    [totalNet.id] NVARCHAR(4000),
    [totalNet.extension] NVARCHAR(MAX),
    [totalNet.value] float,
    [totalNet.currency] NVARCHAR(4000),
    [totalGross.id] NVARCHAR(4000),
    [totalGross.extension] NVARCHAR(MAX),
    [totalGross.value] float,
    [totalGross.currency] NVARCHAR(4000),
...
FROM openrowset (
    ...
    CROSS APPLY openjson (rowset.[participant.JSON]) with (
        ...
        [participant.actor.id]         NVARCHAR(4000)      '$.actor.id',
...
FROM openrowset (
    ...
    CROSS APPLY openjson (rowset.[lineItem.JSON]) with (
        ...
        [lineItem.chargeItem.reference.id] NVARCHAR(4000)      '$.chargeItem.reference.id',
        ...
        [lineItem.chargeItem.codeableConcept.id] NVARCHAR(4000)      '$.chargeItem.codeableConcept.id',
...
FROM openrowset (
    ...
    CROSS APPLY openjson (rowset.[totalPriceComponent.JSON]) with (
        [totalPriceComponent.id]       NVARCHAR(4000)      '$.id',
        ...
        [totalPriceComponent.code.id]  NVARCHAR(4000)      '$.code.id',
        ...
        [totalPriceComponent.amount.id] NVARCHAR(4000)      '$.amount.id',
        [totalPriceComponent.amount.extension] NVARCHAR(MAX)       '$.amount.extension',
        [totalPriceComponent.amount.value] float               '$.amount.value',
        [totalPriceComponent.amount.currency] NVARCHAR(4000)      '$.amount.currency'
    ) j
